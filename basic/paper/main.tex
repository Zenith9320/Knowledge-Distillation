...
\begin{abstract}
Large language models (LLMs) have demonstrated remarkable capabilities in mathematical reasoning, yet their computational requirements pose challenges for practical deployment. Knowledge distillation offers a promising approach to compress these capabilities into smaller models. In this paper, we investigate chain-of-thought (CoT) knowledge distillation for mathematical reasoning, using DeepSeek-R1-Distill-Qwen-1.5B as the teacher and Qwen2.5-0.5B as the student. We explore two complementary strategies for constructing CoT training data: double-temperature generation with semantic deduplication, and adaptive sampling with log-probability confidence scoring. Our experiments on GSM8K and MATH compare these strategies against single-temperature generation, revealing key trade-offs between data quantity, diversity, and downstream performance. We identify a \textit{distillation tax} with instruction-tuned models-where fine-tuning on foreign-style CoT traces degrades native reasoning-and show that switching to a base model eliminates this confound, yielding genuine improvements unambiguously attributable to distillation. We further demonstrate that semantic deduplication is critical for preventing template overfitting while preserving problem coverage, and that multi-temperature generation provides meaningfully diverse reasoning paths that single-temperature multi-sampling cannot replicate.
\end{abstract}
...
In this work, we address these challenges through two complementary strategies for constructing high-quality CoT training data:

\begin{enumerate}[leftmargin=*]
    \item \textbf{Double-Temperature Generation with Semantic Deduplication.} By generating CoT traces at two distinct temperatures ($T = 0.6$ for quality and $T = 0.9$ for diversity) and applying semantic similarity-based deduplication, we expand training coverage while minimizing redundancy-retaining diverse reasoning paths only when they are meaningfully different.
    \item \textbf{Adaptive Sampling with Log-Probability Confidence.} We propose a confidence-guided multi-sampling strategy that uses per-token log-probability scores to dynamically allocate sampling budget: low-temperature generation suffices for high-confidence problems, while high-temperature resampling is reserved for cases where the teacher exhibits uncertainty. This eliminates the computational overhead of traditional self-consistency voting while maintaining high answer quality.
\end{enumerate}

We conduct comprehensive experiments comparing single-temperature, double-temperature with semantic deduplication, and adaptive sampling strategies, providing insights into the trade-offs between data quality, diversity, and computational efficiency.
...
We evaluate on GSM8K (8,500 grade-school math word problems) and MATH (12,500 competition-level problems across seven subjects and five difficulty levels), providing complementary evaluation of basic multi-step reasoning and advanced problem-solving.

\subsection{Adaptive and Multi-Temperature Sampling}

Temperature-based sampling strategies have been widely explored for improving the quality and diversity of LLM-generated text. Self-consistency methods sample multiple reasoning paths at a fixed temperature and select the most frequent answer via majority voting, but incur significant computational overhead. Adaptive temperature strategies aim to allocate sampling budget more efficiently by adjusting temperature based on problem difficulty or model confidence. Our work builds on these ideas by using per-token log-probability scores-which are natively available in modern LLM inference engines-as a lightweight confidence signal, eliminating the need for multiple sampling rounds for confidence estimation.
...
The baseline base model achieves only 13.80\% accuracy on GSM8K and 13.04\% on MATH, with empty-answer rates of 58.0\% and 44.3\% respectively. The MATH empty rate is lower than GSM8K's partly because MATH problems elicit longer reasoning traces before degeneration occurs, and partly because MATH's more structured problem statements provide stronger generation anchoring. The accuracy gap between benchmarks is negligible (0.76 pp), confirming that the untuned base model lacks meaningful mathematical reasoning capability on both simple and complex problems. A closer examination of the GSM8K baseline predictions reveals a deeper issue: 128 of 1,319 samples (9.7\%) exhibit \textbf{degenerative looping}, where the model falls into a repetitive token pattern-outputting the same token or short phrase hundreds of times (e.g., ``moire'' repeated 700+ times, ``RTOS'' repeated 500+ times, or random Unicode fragments such as ``鲕'' and ``离退休'' appearing in English math contexts). This degeneration behavior is characteristic of base models prompted beyond their capabilities, and accounts for a substantial portion of the empty-answer rate: looped outputs rarely contain extractable answers.
